...
public class NewsDAO {
    ...
    public void updateNews(int newsId, int categoryId) {
        try {
            String query = "UPDATE news_agency.news SET ID_CO = " + categoryId + ", NAME = '"  + "' WHERE ID_NE = " + newsId;
            statement.executeUpdate(query);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    public News searchNewsByText(String newsTitle) {
        try {
            ...
            if (resultSet.next()) {
                ...
                resultSet.close();
                return null; // Return null if news is not found
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
